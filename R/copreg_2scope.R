...
#'
#' Implements the two-stage copula generated regressor estimator proposed by
#' Yang et al. (2025).
#'
...
CopReg2sCOPE <- function(formula, data, cdf = c("kde", "ecdf", "resc.ecdf", "adj.ecdf"),
  ...
  fit <- scope_fit(cleaned, components, formula, cdf)
  ...
  make_result(
    ...
    call = call,
    method = "Yang et al. (2025)",
    cdf = cdf,
    boot_replicates = boot_mat
  )
}

scope_fit <- function(data, components, original_formula, cdf) {
  ...
  if (length(exog) == 0) {
    ...
    for (j in seq_along(endog)) {
      ...
      lm_fit <- safe_lm(stats::as.formula(paste(p_var, "~ .")), data = df_reg)
